...
> 目的：解决高频场景（约 99%）：同一 DWG/DXF 内存在多张 A4 说明页（001），其中**只有第 1 张有图签/标题栏信息**，其余页通常只有外框 + 右上角页码/说明。  
...
- Slave Page：簇内其它 A4 页，通常无图签，仅有右上角页码
...
- sy = H_obs / H_canon
- geom_scale = (sx+sy)/2
- fit_error = max(|sx-geom_scale|, |sy-geom_scale|)/max(geom_scale, 1e-6)
...
- gap_threshold 建议：1.0 * min(W_obs, H_obs)（随比例缩放自适应）
取连通分量作为 cluster。
...
- A4_page_marker_rb_offset_1to1 = [dx_right=0, dx_left=120, dy_bottom=255, dy_top=295]
...
取消回填：
- 若 Master 缺失关键字段（如：内部编码/外部编码/标题/张数等），应直接提示错误（不中断全任务，但该 sheet-set 不应再尝试“从 Slave 回填”来掩盖缺失）。

---

## 10. 裁切输出（保持多页 A4 间几何关系不变）

要求：输出DXF→DWG时，各 A4 外框之间的相对距离/排布不变。
