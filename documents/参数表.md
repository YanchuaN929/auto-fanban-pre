...
3. **`catalog_no`（目录编号）来源**：这里的“目录编号”指**目录模板页眉里的“编号/Document No.”字段**，通用模板建议落点为 `目录模板文件.xlsx/Sheet1!H1`，1818 模板落点为 `1818图册目录模板.xlsx/Sheet1!H1`（合并 `H1:I2`）。请你确认该字段的真实来源：用户输入？由 `album_code` 推导？或有固定规则？
...
| `item_name`（子项名称） | 子项名称 | str | 是 | **由 `subitem_no` 查“子项号→子项名称”对照表**（映射表位置待定） |
| `design_phase`（设计阶段） | 设计阶段 | str | 是 | **与你确认：设计阶段就是状态**，因此不单独提取；直接取 `doc_status` / `frames[].titleblock.status`（同一字段复用） |
| `discipline`（专业/工种） | 专业（中文或简称） | str | 是 | **从 DXF 图签“专业”提取**（见 `FrameMeta.titleblock.discipline`） |
| `album_title`（图册/文件名称） | 图册/文件名称（册名） | str | 是 | **待你给出 DXF 图签字段来源**：当前 YAML 未定义该字段；若从“图纸标题/外部编码”等拼接，需要明确规则 |
| `album_code`（图册/文件编号/文档号） | 图册/文件编号（文档号） | str | 是 | **待你给出 DXF 图签字段来源**：当前 YAML 的 `external_code` 为“图纸外部编码”，未必等同“图册/文件编号”；需你确认映射关系 |
| `catalog_no`（目录页眉“编号/Document No.”） | 目录编号 | str | 否 | 你提出“仅 1818 需要且等于项目号”的假设：需要你确认；若为固定值可在规则里默认填充 |
| `album_code`（图册编号） | 图册编号（从内部编码派生） | str | 是 | **不新增ROI**：从 `frames[].titleblock.internal_code` 的“中间5位”的末2位提取（两种 internal_code 格式都适用），例如 `20161NH-JGS01 -> 01` |
| `volume_index`（册次-第几册） | 第几册 | int | 否 | 当前默认 1（你已说明暂时固定） |
| `volume_total`（册次-共几册） | 共几册 | int | 否 | 当前默认 1（与 `volume_index` 组合生成“共X册 第Y册”） |
| `revision`（版次/Rev.） | 版次/Rev. | str | 是 | **从 DXF 图签“版次”提取**（见 `FrameMeta.titleblock.revision`） |
| `doc_status`（状态/Status） | 状态/Status | str | 是 | **从 DXF 图签“状态”提取**（见 `FrameMeta.titleblock.status`） |
| `classification`（密级） | 密级/Classification | str | 是 | **前端输入**；写入封面/设计文件/IED 的密级字段 |
| `approved_by`（封面批准人） | 批准人 | str | 否 | 你标注“封面文本不作操作”与“封面几乎所有内容动态写入”矛盾：建议确认是否需要写入模板 |
| `output_locale`（输出语言模式） | 输出语言 | enum | 否 | 若模板为双语且无需程序翻译，则可仅用于**决定是否保留/隐藏英文栏位**；否则可移除该变量（你已提出） |
...
- 取 `internal_code_001 = frames[internal_code endswith '-001'].titleblock.internal_code`
- 目录（Catalog）内部编码：将 `internal_code_001` 的末段 `-001` 替换为 `-TM`
  - `catalog_internal_code = <prefix>-TM`（prefix 为 `XXXXXXX-XXXXX`，与 001 图纸保持一致）
- 封面（Cover）内部编码：将 `internal_code_001` 的末段 `-001` 替换为 `-FM`
  - `cover_internal_code = <prefix>-FM`
...
| `frames[].titleblock.subitem_no`（子项号） | 子项号 | `field_definitions.subitem_no` | 子项号 | `text_exact_or_lexicon` | **封面/目录使用**；并可查表得到 `item_name` |
...
| `frames[].titleblock.title_cn`（中文标题） | 图纸标题 | `field_definitions.title_cn` | 图纸标题 | `text_multiline` | 目录：名称(E) / 设计文件：中文标题(G) / IED：中文标题(K) |
...
| `album_code` | 图册编号 | `I25` | 标签 `D25`（你确认此处就是图册编号落点，统一命名为 code） |
...
| `album_code` | 图册编号/Album Code | `I25` | 标签 `D25/D26`（与通用封面一致） |
...
| `catalog_no`（目录页眉编号=内部编码TM） | 编号 | `H1` | **内部编码**：取 001 图纸内部编码派生 `-TM`（见 2.3.0） |
| `album_code`（文件编号=外部编码T01） | 文件编号 | `H3` | **外部编码**：取 001 图纸外部编码并将第 9~11 位替换为 `T01`（见 2.3.0） |
| `subitem_no`（子项号） | 子项号 | `C5` | 标签 `A5`（封面/目录使用子项号） |
| `revision`（版次） | 版次 | `H5` | 标签 `G5` |
...
| E | 图纸（文件）名称 | `frames[].titleblock.title_cn`（或规则拼接） |
| F | 版次 | `frames[].titleblock.revision`（或聚合全局） |
| G | 状态 | `frames[].titleblock.status`（或聚合全局） |
| H | 页数 | `frames[].titleblock.page_total`（若无则默认 1） |
| I | 附注 | （待定：目前 YAML 未定义“附注”字段，若需要请补 ROI 定义或置空） |
...
| `catalog_no`（目录页眉编号/Document No.） | Document No. | `H1`（合并 `H1:I2`） |
| `album_code`（文档号/Document Code） | Document Code | `H3`（合并 `H3:I4`） |
| `subitem_no`（子项号/Item No.） | Item No. | `C5`（合并 `C5:C6`）（目录使用子项号） |
| `revision`（版次/Rev.） | Rev. | `H5`（合并 `H5:H6`） |
...
| 列 | 表头 | 参数Key（建议） | 推荐来源 |
|---:|---|---|---|
| A | 状态 | `design.status`（设计文件条目状态：新增/变更/冻结等） | 规则/默认（需你定枚举） |
| B | WBS编码 | `design.wbs_code`（WBS分解编码：用于项目管理/归档） | 规则/映射（需你提供规则或映射表） |
| C | 所属图册编码 | `album_code`（图册/文档号：该文件归属的图册） | 全局/规则（建议由 DXF/规则聚合得到，见 2.2） |
...
| G | 中文标题 | `frames[].titleblock.title_cn`（中文标题） | DXF图签提取 |
| H | 英文标题 | `title_en`（英文标题） | 目前 YAML 未定义英文标题字段（需补充或留空） |
| I | 内部标识 | `design.internal_tag`（内部标识：内部管理用的附加标识） | 规则/可空 |
| J | 子项名称 | `item_name`（子项名称/系统名称） | 全局/规则 |
| K | 子项号 | `subitem_no`（子项号：来自 DXF 图签） | 全局/规则 |
| L | 系统代码 | `system_code`（系统代码：前端输入，默认 NA） | 全局（前端输入） |
| M | 系统名称 | `system_name`（系统名称：前端输入，默认 NA） | 全局（前端输入） |
| N | 专业(工种) | `frames[].titleblock.discipline`（专业/工种） | DXF图签提取（或聚合全局） |
| O | 专业代码 | `design.discipline_code`（专业对应的标准代码） | 映射表（需你给“专业→代码”） |
| P | 专业室 | `design.discipline_office`（专业室/责任单位子项） | 映射表/可空 |
| Q | 设计阶段 | `design_phase`（设计阶段：按你确认=状态） | 直接复用 `frames[].titleblock.status`（同 `doc_status`） |
| R | 密级 | `classification`（密级：内部/公开等） | 全局/规则 |
| S | 图幅 | `frames[].titleblock.paper_size_text`（图幅规格：A0+1/2等） | DXF图签提取 |
| T | 总页数 | `frames[].titleblock.page_total`（页数：默认1） | DXF图签提取（无则默认 1） |
| U | 文件类别 | `design.file_category`（文件类别：按模板“文件类别”sheet的分类） | 规则/映射 |
| V | 图纸文件状态 | `design.drawing_status`（图纸文件状态：业务状态枚举） | 规则/映射 |
| W | 附件名称 | `design.attachment_name`（附件名称：若有外部附件） | 可空 |
| X | 是否质保核查 | `design.qa_required`（是否需要质保核查：是/否） | 规则/默认 |
| Y | 质保核查工程师 | `design.qa_engineer`（质保核查责任人） | 规则/可空 |
| Z | 实际工时数 | `design.actual_hours`（实际工时：数值） | 可空（数值） |
...
> IED字段释义（括号内）：
> - 新增标记（A：填写“A”代表新增导入）
> - 变更标记（B：MOD/CAN/REC/IDM 等变更类型，见模板“填写说明”）
> - 文档类型（C：图册/文件/图纸/章节等类型）
> - IED状态（D：编制/发布等状态，影响是否驱动生成设计文件）
> - WBS编码（E：系统中存在的WBS码）
> - 所属图册/文件编码（F：当类型为图纸/章节时，归属的图册或联合编审文件外部编码）
> - 文件编码（G：外部编码/唯一标识）
> - 原文件编码（H：IDM 模式下填原错误编码）
> - 内部编码（I：内部管理编码；也可用于排序/关联）
> - 版本（J：版次/Rev）
> - 中文标题（K：中文名称）
> - 英文标题（L：英文名称）
> - 系统代码（AE：取前端输入 `system_code`，默认 `NA`）
> - 其余列（M~BN：资格/计划/安装/土建/校审链条/原因等扩展信息，后续按你提供的“字段策略”决定必填与生成规则）
