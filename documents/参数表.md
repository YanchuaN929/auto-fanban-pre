...
| `album_code`（图册编号） | 图册编号（末2位数字） | str | 是 | **不新增ROI**：从 `frames[].titleblock.internal_code` 的“中间5位”末2位提取（两种 internal_code 格式都适用），例如 `20161NH-JGS01 -> 01`；用于封面 I25（合并 `I25:S25`） |
...
| `volume_total` + `volume_index` | 共X册 第Y册 | `N4` | **暂时固定不输入/不改动**（模板默认已填） |
...
| `subitem_name_EN` | 子项名称-英文翻译/Item Name | `I15` | 标签 `D15` |
| `design_phase` | 设计阶段/Design Phase | `I16` | 标签 `D16` |
| `design_phase_EN` | 设计阶段-英文翻译/Design Phase | `I17` | 标签 `D17` ，当design_phase为“施工图设计”时，design_phase_EN值为“Constructing Design”|
| `discipline` | 专业/Discipline | `I18` | 标签 `D18` |
| `discipline_EN` | 专业/Discipline | `I18` | 标签 `D19` 该值为专业的英文翻译，我给出对照：结构-strcture\建筑-Architectural|
...
| `volume_total` + `volume_index` | Vol. x of y | `M4` | 例如：`Vol. 1 of 3`，同通用模板固定值，这里不做变量处理 |
...
| I | 附注 | **升版标记**：前端输入“升版范围 start_seq~end_seq”（例如 005~012），则对排序后的图纸行中尾号在该范围内的行，I列写入文本 `升版`；其余留空（封面/目录行留空） |
...
| J | 子项名称 | `subitem_name`（子项名称） | 全局/规则 |
