...
> 运行期参数（并发/超时/ODA路径等）见：`documents/参数规范_运行期.yaml`
...
| `pdf_margin`（PDF页边距设置，适用于DWG/DXF打印） | 上下左右页边距（mm） | dict | 默认 `{top:20, bottom:10, left:20, right:10}`，定义为“PDF 页面边缘 → 图框最大外框线”的距离 |
...
- 优先使用 Excel 分页信息（如 `HPageBreaks/VPageBreaks`）计算页数，回填 `H` 列后仅导出一次最终 PDF。
- 若分页信息不稳定，则回退为导出PDF计页再回填的双导出流程，确保 `xlsx` 与 `pdf` 一致。
