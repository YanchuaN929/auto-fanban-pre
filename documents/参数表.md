...
3. **`catalog_internal_code`（目录编号/Document No.）来源**：这里的“目录编号”指**目录模板页眉里的“编号/Document No.”字段**，通用模板建议落点为 `目录模板文件.xlsx/Sheet1!H1`，1818 模板落点为 `1818图册目录模板.xlsx/Sheet1!H1`（合并 `H1:I2`）。已固化规则：取 `catalog_internal_code`（由 001 图纸内部编码把 `-001` 替换为 `-TM` 派生）。
- 答复：待你进一步明确（请落实到前端字段名称与示例规则）。
...
- `ied_xlsx, ied_pdf`（IED 计划 Excel 与对应 PDF）
...
| `ied_verify_by`（验证人） | 验证人 | str | 发布时必填 | 前端输入；格式 `姓名@ID`（填写说明） |
| `ied_verify_date`（验证日期） | 验证日期 | str | 发布时必填 | 前端输入；格式 `YYYY-MM-DD` 且必须为文本格式（填写说明） |
...
| `ied_plan_countersign_date`（计划会签日期） | 计划会签日期 | str | 否 | 前端输入；日期格式 `YYYY-MM-DD` 且必须为文本格式（填写说明） |
...
| `ied_planned_wr_date`（计划WR日期） | 计划WR日期 | str | 否 | 前端输入；建议 `YYYY-MM-DD` 文本格式 |
| `ied_person_qual_category`（人员资格类别） | 人员资格类别 | str | 发布时必填 | **前端输入**：发布状态下必填；具体资格校验规则见模板 `IED计划模板文件.xlsx` 的 `设计资格校验规则` |
| `ied_planned_hours`（计划工时） | 计划工时 | str | 发布时必填 | **前端输入**：发布状态下必填；默认 `100`（后续可按规则/映射表细化） |
| `subitem_name`（子项名称） | 子项名称 | str | 是 | **由 `subitem_no` 查“子项号→子项名称”对照表**（映射表位置待定） |
...
| `frames[].titleblock.subitem_no`（子项号） | 子项号 | `field_definitions.subitem_no` | 子项号 | `text_exact_or_lexicon` | **封面/目录使用**；并可查表得到 `subitem_name` |
...
> 备注：封面内部编码（`cover_internal_code`，`-FM` 规则）与目录内部编码（`catalog_internal_code`，`-TM` 规则）在模板中的具体单元格位置，你还未提供；我先把命名规则固化在 2.3.0，等你给出落点后再补齐“写入单元格”。
...
| `subitem_name` | 子项名称 | `I14` | 标签 `D14` |
| `design_phase` | 设计阶段/Design Phase | `I16` | 标签 `D16` |
| `discipline` | 专业/Discipline | `I18` | 标签 `D18` |
> 1818 封面涉及的英文翻译（子项名称/设计阶段/专业）建议由**映射表**生成并写入对应单元格（不作为全局参数 Key 固化；后续如需固化成 Key 再补进 `参数规范.yaml`）。
...
| 列 | 表头 | 参数Key（建议） | 推荐来源 |
|---:|---|---|---|
| A | 状态 | `design.status`（设计文件状态：这个需要区别于图纸、封面、目录的状态status，虽然文件内写的是“状态”但这是专指设计文件.xlsx内种每个图纸的发布状态，默认为“编制”） | 规则/默认：用户前端输入，默认为“编制” |
...
| I | 内部标识 | `design.internal_tag`（内部标识：内部管理用的附加标识） | 规则:用户前端输入，默认为“否”|
| J | 子项名称 | `subitem_name`（子项名称） | **派生**：由 `subitem_no` 查“子项号→子项名称”对照表（见 2.2） |
| K | 子项号 | `subitem_no`（子项号） | **titleblock 提取**：来自 `frames[].titleblock.subitem_no`（全局聚合见 2.2） |
| L | 系统代码 | `system_code`（系统代码：前端输入，默认 NA） | 全局（前端输入） |
| M | 系统名称 | `system_name`（系统名称：前端输入，默认 NA） | 全局（前端输入） |
...
| P | 专业室 | `design.discipline_office`（专业室/责任单位子项） | 用户前端输入，默认为空|
| Q | 设计阶段 | `design_phase`（设计阶段） | **派生**：来自 `frames[].titleblock.status`；当 status=`CFC` 时映射为 **“施工图设计”**（见 2.2） |
| R | 密级 | `classification`（密级：内部/公开等） | 全局/规则 ：前端用户输入，默认为“非密”|
...
| U | 文件类别 | `design.file_category`（文件类别） | **前端选择输入**：选项集合来自 `设计文件模板.xlsx` 的 Sheet5 “文件类别” A列 |
| V | 图纸文件状态 | `frames[].titleblock.status`（状态） | **不新增变量**：与 titleblock.status 同值。封面/目录行：取 001 图纸的 status（=全局 `doc_status`） |
| W | 附件名称 | `design.attachment_name`（附件名称：若有外部附件） | 规则：用户前端输入，默认为空 |
| X | 是否质保核查 | `design.qa_required`（是否需要质保核查：是/否） | 规则：用户前端输入，默认为“否” |
| Y | 质保核查工程师 | `design.qa_engineer`（质保核查责任人） | 规则：用户前端输入，默认为空 |
| Z | 实际工时数 | `design.actual_hours`（实际工时：数值） | 规则：用户前端输入，默认为“100” |
...
| B | 变更标记 | `ied.change_flag` | 前端选择/可空（MOD/CAN/REC/IDM…，见模板“填写说明”） |
| C | 文档类型 | `ied.doc_type` | 前端选择（图册/文件/图纸/章节…） |
...
| F | 所属图册/文件编码 | `ied.parent_external_code` | **待你确认规则**：当类型为图纸/章节时，归属的图册或联合编审文件外部编码 |
| G | 文件编码 | `frames[].titleblock.external_code`（外部编码） | 图纸行：DXF提取；封面/目录行：`cover_external_code` / `catalog_external_code` |
| H | 原文件编码 | `ied.original_external_code` | 规则：前端输入/可空（IDM 模式下填原错误编码） |
...
| M | 人员资格类别 | `ied_person_qual_category` | **发布状态必填**（填写说明）。前端输入（校验规则见模板“设计资格校验规则”）。 |
| N | 所提交计划 | `ied_submitted_plan_date` | 日期格式 `YYYY-MM-DD` 且必须为文本格式（填写说明）。 |
| O | 计划会签日期 | `ied_plan_countersign_date` | 日期格式 `YYYY-MM-DD` 且必须为文本格式（填写说明）。 |
| P | 出版计划 | `ied_publish_plan_date` | 日期格式 `YYYY-MM-DD` 且必须为文本格式（填写说明）。 |
| Q | 外部计划 | `ied_external_plan_date` | 日期格式 `YYYY-MM-DD` 且必须为文本格式（填写说明）。 |
| R | FU标记 | `ied.fu_flag` | （一期留空/可选）前端输入/默认 |
| S | FU计划 | `ied_fu_plan_date` | 日期格式 `YYYY-MM-DD` 且必须为文本格式（填写说明）。 |
| T | 计划调整依据 | `ied.plan_adjust_basis` | （一期留空/可选）前端输入 |
| U | 内部标识 | `ied.internal_tag` | （一期留空/可选）前端输入/默认 |
| V | 设计类型 | `ied.design_type` | （一期留空/可选）前端选择（可参考模板内设计类型相关sheet） |
| W | 责任设总 | `ied.chief_designer` | （一期留空/可选）前端输入 |
| X | 责任单位 | `ied.responsible_unit` | （一期留空/可选）前端输入 |
| Y | 是否联合编审文件 | `ied.is_joint_review_doc` | （一期留空/可选）前端选择（是/否） |
| Z | 章节号 | `ied.chapter_no` | （一期留空/可选）前端输入 |
| AA | 章节名称 | `ied.chapter_name` | （一期留空/可选）前端输入 |
| AB | SSC编码 | `ied.ssc_code` | （一期留空/可选）前端输入/映射 |
| AC | 是否更新SSC关联 | `ied.update_ssc_link` | （一期留空/可选）前端选择（是/否） |
| AD | 机组 | `ied.unit_no` | （一期留空/可选）前端选择/默认 |
| AE | 系统代码 | `system_code`（系统代码） | 全局：前端输入，默认 `NA` |
| AF | 安装厂房 | `ied.install_building` | （一期留空/可选）前端输入/映射 |
| AG | 安装层位及标高 | `ied.install_level_elev` | （一期留空/可选）前端输入 |
| AH | 安装区域 | `ied.install_area` | （一期留空/可选）前端输入 |
| AI | 安装层位 | `ied.install_level` | （一期留空/可选）前端输入 |
| AJ | 岛别 | `ied.island` | （一期留空/可选）前端输入/映射 |
| AK | 土建厂房代码 | `ied.civil_building_code` | （一期留空/可选）前端输入/映射 |
| AL | 土建分区 | `ied.civil_zone` | （一期留空/可选）前端输入/映射 |
| AM | 土建层位及标高 | `ied.civil_level_elev` | （一期留空/可选）前端输入 |
| AN | 土建区域 | `ied.civil_area` | （一期留空/可选）前端输入 |
| AO | 房间 | `ied.room` | （一期留空/可选）前端输入 |
| AP | 模块 | `ied.module` | （一期留空/可选）前端输入 |
| AQ | 设备代码 | `ied.equipment_code` | （一期留空/可选）前端输入/映射 |
| AR | 总体性文件 | `ied.is_overall_doc` | （一期留空/可选）前端输入/默认 |
| AS | 相关物项类别 | `ied.related_item_category` | （一期留空/可选）前端输入/映射 |
| AT | 计划工时 | `ied_planned_hours` | **发布状态必填**（填写说明）。前端输入，默认 `100`（同值回填到所有行）。 |
| AU | 是否验证 | `ied.is_verified` | （一期留空/可选）前端选择（是/否） |
| AV | 验证方法 | `ied.verify_method` | （一期留空/可选）前端输入 |
| AW | 验证人 | `ied_verify_by` | **发布状态必填**：格式 `姓名@ID`（填写说明）。 |
| AX | 验证日期 | `ied_verify_date` | **发布状态必填**：日期格式 `YYYY-MM-DD` 且必须为文本格式（填写说明）。 |
| AY | 编制者 | `ied_prepared_by` | **发布状态必填**：格式 `姓名@ID`（填写说明）。 |
| AZ | 第二编制者 | `ied_prepared_by_2` | 格式 `姓名@ID`（填写说明）。 |
...
| BD | 工种负责人 | `ied_discipline_leader` | 格式 `姓名@ID`（填写说明）。 |
| BE | 工种负责人审核日期 | `ied_discipline_leader_date` | 日期格式 `YYYY-MM-DD` 且必须为文本格式（填写说明）。 |
| BF | 审核者 | `ied_reviewed_by` | **发布状态必填**：格式 `姓名@ID`（填写说明）。 |
| BG | 审核日期 | `ied_reviewed_date` | **发布状态必填**：日期格式 `YYYY-MM-DD` 且必须为文本格式（填写说明）。 |
| BH | 审定者 | `ied_approved_by` | 格式 `姓名@ID`（填写说明）。 |
| BI | 审定日期 | `ied_approved_date` | 日期格式 `YYYY-MM-DD` 且必须为文本格式（填写说明）。 |
| BJ | 专业室 | `ied.discipline_office` | （一期留空/可选）前端输入/映射 |
| BK | 密级 | `classification`（密级） | 全局：前端输入 |
| BL | 所提交计划原因 | `ied.submitted_plan_reason` | （一期留空/可选）前端输入 |
| BM | 出版计划原因 | `ied.publish_plan_reason` | （一期留空/可选）前端输入 |
| BN | 外部计划原因 | `ied.external_plan_reason` | （一期留空/可选）前端输入 |
| BO | 预估图纸量 | `ied.estimated_drawing_count` | （一期留空/可选）前端输入 |
| BP | 原定费用(元) | `ied.original_cost_cny` | （一期留空/可选）前端输入 |
| BQ | 计划WR日期 | `ied_planned_wr_date` | 日期类字段，建议 `YYYY-MM-DD` 文本格式。 |
| BR | 是否三维模型 | `ied.has_3d_model` | （一期留空/可选）前端选择（是/否） |
| BS | 是否交付数据 | `ied.deliver_data` | （一期留空/可选）前端选择（是/否） |
| BT | 数据类型 | `ied.data_type` | （一期留空/可选）前端选择/映射（见模板相关字段/说明） |
| BU | EM包 | `ied.em_package` | （一期留空/可选）前端输入/映射 |
| BV | 安装大类 | `ied.install_category` | （一期留空/可选）前端输入/映射 |
| BW | 责任系统 | `ied.responsible_system` | （一期留空/可选）前端输入/映射 |
...
> 建议落地策略（避免一次性把 75 列都“强必填”）：  
> - **第一期只强制**：`文档类型/IED状态/WBS编码/所属图册或文件编码/文件编码/内部编码/版本/中文标题`（其余列可留空或走规则默认）。  
> - 第二期再逐步补齐：安装/土建/SSC/校审链条/原因等字段（通过映射表或业务规则生成）。
