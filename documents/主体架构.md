...
> `DWG→DXF → 识别图框 → 图签字段提取 →（A4多页成组）→ 拆分/裁切 → 重命名 → 输出PDF+DWG → 生成封面/目录/设计文件并导出PDF → 打包ZIP + 单独输出IED计划.xlsx`  
> 并提供“词库检查/替换（纠错/翻版）”能力。
>
> **硬约束**：
> - **DXF 仅作为中间文件**，不对用户输出。
> - 首版可跳过字段/XREF（外部参照），不追求完全保真，但需支持 **多图幅、多比例混排**。
> - 文档（封面/目录/设计文件）**必须导出 PDF**；IED 仅 Excel 单独输出，不导出 PDF。
>
> **配置源事实（权威）**：`documents/参数规范.yaml`（v2.0）  
> **运行期参数**：`documents/参数规范_运行期.yaml`（并发/超时/路径/清理等）
...
- **`documents/参数规范_运行期.yaml`**：运行期参数（并发、超时、ODA路径、清理策略等）
...
**输出（下载）**
- 单个 `package.zip`（交付包）
  - 拆分后的图纸产物（PDF + DWG，DWG 必须包含）
  - 封面：docx + pdf
  - 目录：xlsx + pdf
  - 设计文件：xlsx + pdf
  - `manifest.json`（机器可读清单：输入、派生、落点、告警 flags、生成日志摘要）
- 额外输出：`IED计划.xlsx`（单独一个 Excel 文件，不包含在 package.zip 中，也不导出 PDF）
...
- `options`：`doc_generation.options` + 运行期参数（见 `documents/参数规范_运行期.yaml`）
...
### 3.5 `manifest.json` 结构示例（建议）
> 目的：保证前端/审计/回溯一致性，建议强制输出。
...
```json
{
  ...
  "project_no": "2016",
  "spec_version": "documents/参数规范.yaml@2.0",
  "runtime_spec_version": "documents/参数规范_运行期.yaml@1.0",
  "inputs": {
    "dwg_files": ["a.dwg", "b.dwg"],
    "doc_generation_options": { "export_pdf": true, "pdf_engine": "office_com" }
  },
  "derived": {
    "cover_internal_code": "...-FM",
    "catalog_internal_code": "...-TM",
    "catalog_page_total": 12
  },
  "artifacts": {
    "package_zip": "storage/jobs/{job_id}/package.zip",
    "ied_xlsx": "storage/jobs/{job_id}/ied/IED计划.xlsx",
    "drawings_dir": "storage/jobs/{job_id}/output/drawings",
    "docs_dir": "storage/jobs/{job_id}/output/docs"
  },
  "flags": [],
  "errors": [],
...
9. `EXPORT_PDF_AND_DWG`：输出子图 PDF（按图幅/页边距）与 DWG（DXF→DWG，必须）
10. `GENERATE_DOCS`：封面/目录/设计文件生成并导出 PDF；IED 生成 Excel 单独输出（不导出 PDF）
...
- `enabled`: true 时 **封面/目录/设计文件生成并导出 PDF；IED 仅 Excel 单独输出**
- `export_pdf`: 必须 true
- `pdf_engine`: `office_com` 优先（含 OLE 时更可靠），`libreoffice` 备选
- `pdf_margin_mm`: DXF→PDF 打印页边距（四边，单位mm，指 PDF 页面边缘到图框最大外框线的距离；默认 top/bottom/left/right = 20/10/20/10）
...
- **1818 封面落点与通用封面不同（整体上移一行）**，必须按 `doc_generation.templates.cover_bindings` 分支读取落点，严禁硬编码。
...
2. 动态设置打印区域（`Print_Area/Print_Titles`）
3. **优化方案**：使用 Excel 分页信息计算页数（例如 `HPageBreaks/VPageBreaks` 计数）得到 `catalog_page_total`，回填目录行 `H` 列，再导出一次 PDF（只导出一次最终 PDF）
4. **兜底方案**：若分页信息不稳定或兼容问题，回退为“先导出 PDF 计页 → 回填 → 再导出”的双导出流程，确保 xlsx 与 pdf 一致
...
IED 输出仅 Excel（单独输出，不导出 PDF，不包含在 package.zip）。
...
- `GET /api/jobs/{job_id}/artifacts/{name}`（单个报表/单个 PDF/IED 计划 Excel）
...
### 11.3 离线部署方案 A（绿色包，无需目标机联网安装依赖）
**适用条件**：目标机已安装 Office 2016+（用于 `office_com` 导出）。
...
deploy/
  app/                      # 后端可执行文件/脚本
  config/
    参数规范_运行期.yaml     # 运行期配置（路径/并发/超时）
  documents/
  documents_bin/
  oda/                      # ODA File Converter 目录（复制过来）
```

运行期配置示例（相对路径）：
```yaml
oda_converter:
  exe_path: ".\\oda\\ODAFileConverter.exe"
pdf_engine:
  preferred: "office_com"
  fallback: "libreoffice"   # 可选：无 LibreOffice 时可忽略
```
...
storage/jobs/{job_id}/
  ...
  work/
    ...
    a4_sheet_sets/
  output/
    drawings/             # 拆分后的 PDF + DWG（DWG 必须包含）
    docs/                 # cover/catalog/design + pdf
    reports/
  ied/                    # IED计划单独输出（不入package.zip、不导出PDF）
  manifest.json
  logs/job.log
  package.zip
```
...
6. A4 多页成组（001 高频场景）
7. 文档生成（封面/目录/设计文件）+ PDF 导出；IED Excel 单独输出（Office COM 优先）
